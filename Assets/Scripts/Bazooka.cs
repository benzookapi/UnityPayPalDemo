               ...
               if (bullet != null && spawm != null)
               {
                   ...
                   if (rb != null)
                   {
                      rb.velocity = spawm.transform.TransformDirection(Vector3.forward * 20);
											...
											Score.cur_count += 1;
											if (Score.cur_count % Score.charge_count == 0)
											{
												int charged = Mathf.Abs(Score.amt) * Score.charge_rate / 100;
												Score.amt = Score.amt - charged;
												string url = "https://jo-pp-ruby-demo.herokuapp.com/unity/charge?ba=" + LoginForm.accessKey + "&amt=" + charged;
												Debug.Log("Charged! " + url);
												new WWW(url);
											}
